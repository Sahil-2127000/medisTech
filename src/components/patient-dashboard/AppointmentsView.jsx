...
const AppointmentsView = ({ appointments = [], onBookClick }) => {
    ...
    const [selectedPatient, setSelectedPatient] = useState('All Family');
    ...
    const handleFilterChange = (newFilter) => {
        setFilter(newFilter);
        navigate(`/patientdashboard/calendar?filter=${newFilter}`, { replace: true });
    };
    ...
    // Extract unique patient names for the selector
    const allPatientNames = ["All Family", ...new Set(sortedAppointments.map(app => app.patientName || "Primary Account"))];

    const filteredAppointments = sortedAppointments.filter(app => {
        const matchesStatus = filter === 'all' || (filter === 'accepted' ? (app.status === 'approved' || app.status === 'emergency_active') : app.status === filter);
        const appPatientName = app.patientName || "Primary Account";
        const matchesPatient = selectedPatient === 'All Family' || appPatientName === selectedPatient;
        return matchesStatus && matchesPatient;
    });

    // 2. Group sorted and filtered appointments by patient (family members)
        ...
        <div className="flex-1 h-full py-8 md:py-12 px-6 md:px-12 flex flex-col overflow-y-auto no-scrollbar bg-transparent">
            ...
            <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                ...
                <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={onBookClick}
                    className="bg-linear-to-r from-[#5265ec] to-[#6366f1] text-white px-10 py-3.5 rounded-full font-black text-[11px] uppercase tracking-widest shadow-xl shadow-[#5265ec]/40 transition-all flex items-center gap-2 border border-white/20"
                >
            ...
            {/* Filter Buttons & Family Selector */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex flex-wrap items-center gap-3">
                    {[
                        ...
                        <button
                            ...
                            className={`px-5 py-2 rounded-full font-black text-[10px] uppercase tracking-widest transition-all ${filter === f.id ? 'bg-slate-800 text-white dark:bg-white dark:text-slate-900 shadow-md scale-105' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400 dark:hover:bg-slate-700'}`}
                        >
                            {f.label}
                        </button>
                    ))}
                ...
                <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 p-1.5 rounded-full border border-slate-200/60 dark:border-slate-700 shadow-inner">
                    <div className="bg-white dark:bg-slate-700 w-8 h-8 rounded-full flex items-center justify-center text-[#5265ec] shadow-sm shrink-0">
                    ...
                    <div className="relative group">
                        <select
                            ...
                            className="bg-transparent text-slate-800 dark:text-white text-[11px] uppercase tracking-wider font-black outline-none cursor-pointer appearance-none pr-8 pl-2 py-1 w-[130px] hover:text-[#5265ec] transition-colors"
                        >
                            {allPatientNames.map((name, idx) => (
                                <option key={idx} value={name} className="text-sm font-bold text-slate-800">{name === "Primary Account" ? "Me (Primary)" : name}</option>
                            ))}
                        </select>
                        <div className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400 group-hover:text-[#5265ec] transition-colors">
                                ...
                                {groupedAppointments[name].map((app, appIdx) => {
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            className={`${styles.card} border rounded-3xl p-3 md:p-4 transition-all flex flex-col md:flex-row md:items-center gap-4 group cursor-pointer relative overflow-hidden`}
                                        >
                                            {/* Status Accent Bar */}
                                            <div className={`absolute left-0 top-0 bottom-0 w-2 ${app.status === 'approved' ? 'bg-emerald-500' : app.status === 'pending' ? 'bg-amber-500' : app.status === 'completed' ? 'bg-blue-500' : 'bg-red-500'}`}></div>

                                            {/* Doctor Avatar & Name */}
                                            <div className="flex items-center gap-3 md:w-1/3 relative z-10 pl-2">
                                                <div className="relative shrink-0">
                                                    <img
                                                        src={app.img || `https://ui-avatars.com/api/?name=${encodeURIComponent(app.doctor)}&background=5265ec&color=fff&bold=true`}
                                                        alt="doctor"
                                                        className="w-12 h-12 rounded-xl object-cover border-2 border-white dark:border-slate-800 shadow-md group-hover:scale-110 transition-transform duration-500"
                                                    />
                                                </div>
                                                <div>
                                                    <div className={`text-[9px] font-black uppercase tracking-widest ${styles.subtext}`}>Consultant</div>
                                                    <div className={`text-base font-black transition-colors leading-tight ${styles.text}`}>{app.doctor}</div>
                                                </div>
                                            </div>

                                            {/* Schedule Info */}
                                            <div className="flex items-center gap-6 md:flex-1 relative z-10">
                                                <div className="flex items-center gap-2.5">
                                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors shadow-sm ${styles.icon}`}>
                                                    ...
                                                    <div>
                                                        <div className={`text-[9px] font-black uppercase tracking-widest ${styles.subtext}`}>Date</div>
                                                        <div className={`text-sm font-black ${styles.text}`}>{app.date}</div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2.5">
                                                    ...
                                                    <div>
                                                        <div className={`text-[9px] font-black uppercase tracking-widest ${styles.subtext}`}>Time</div>
                                                        <div className={`text-sm font-black ${styles.text}`}>{app.time}</div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between md:justify-end md:w-1/4 gap-4 relative z-10">
                                                <div className={`px-4 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-widest shadow-xs ${styles.badge}`}>
