...
const EmergencyCase = ({ onDeclareEmergency, onResolveEmergency }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto no-scrollbar py-12 px-8 flex justify-center bg-[#fafcff] dark:bg-slate-800 transition-colors duration-300">
      <div className="w-full max-w-2xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[2.5rem] shadow-[0_15px_40px_rgba(0,0,0,0.03)] p-10 relative overflow-hidden transition-colors duration-300">
        ...
        {isEmergencyActive ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center relative z-10">
            ...
            <h2 className="text-4xl font-extrabold text-red-600 dark:text-red-500 mb-2 uppercase tracking-wide">Emergency In Progress</h2>
            <p className="text-slate-600 dark:text-gray-300 font-bold mb-8">All standard queue appointments are frozen.</p>

            <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/50 rounded-2xl p-6 w-full max-w-sm mb-10 transition-colors">
              <span className="text-xs font-bold text-red-400 uppercase tracking-widest block mb-2">Automated Stopwatch</span>
              <div className="text-6xl font-black text-red-600 dark:text-white font-mono tracking-tighter">
                {formatTime(elapsed)}
              </div>
            </div>
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-4 mb-10 pb-6 border-b border-gray-50 dark:border-slate-800 transition-colors">
              <div className="w-14 h-14 bg-red-100 dark:bg-red-900/30 text-red-500 flex items-center justify-center rounded-2xl shadow-sm transition-colors">
              ...
              <div>
                <h2 className="text-3xl font-extrabold text-[#021024] dark:text-white mb-1 transition-colors">Declare Emergency</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <input value={name} onChange={e=>setName(e.target.value)} required placeholder="Jane Doe / Unknown" className="w-full bg-[#f4f7fb] dark:bg-slate-800 border border-transparent focus:bg-white dark:focus:bg-slate-900 focus:border-red-500 rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 dark:text-gray-100 transition-all outline-none"/>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="number" value={age} onChange={e=>setAge(e.target.value)} required placeholder="e.g. 45" className="w-full bg-[#f4f7fb] dark:bg-slate-800 border border-transparent focus:bg-white dark:focus:bg-slate-900 focus:border-red-500 rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 dark:text-gray-100 transition-all outline-none"/>
                </div>

                <div className="space-y-1.5">
                  <label className="text-[10px] font-extrabold uppercase tracking-widest text-red-500/80 transition-colors">Gender</label>
                  <select value={gender} onChange={e=>setGender(e.target.value)} required className="w-full bg-[#f4f7fb] dark:bg-slate-800 border border-transparent focus:bg-white dark:focus:bg-slate-900 focus:border-red-500 rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 dark:text-gray-100 transition-all outline-none">
                ...
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <input value={reason} onChange={e=>setReason(e.target.value)} required placeholder="e.g. Severe chest pain, unresponsive" className="w-full bg-[#f4f7fb] dark:bg-slate-800 border border-transparent focus:bg-white dark:focus:bg-slate-900 focus:border-red-500 rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 dark:text-gray-100 transition-all outline-none"/>
                </div>
              </div>
