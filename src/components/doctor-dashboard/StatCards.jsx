...
const StatCards = ({ appointments, historicalTotal, onCardClick }) => {
    ...
    const stats = [
        ...
        { id: 'todays-consults', title: "Today's Consults", count: todayAppointments, color: 'bg-teal-400', icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /> },
        { id: 'completed', title: 'Completed', count: completed, color: 'bg-green-500', icon: <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /> },
        { id: 'pending', title: 'Pending Requests', count: pending, color: 'bg-amber-400', icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /> }
    ];
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4 w-full">
            {stats.map((stat, i) => (
                <div key={i} onClick={() => stat.id && onCardClick && onCardClick(stat.id)}
                    className={`bg-white/60 backdrop-blur-md rounded-2xl border border-white/60 p-4 flex items-center gap-4 transition-all relative overflow-hidden shadow-xl shadow-clinic-600/20 hover:-translate-y-1 hover:shadow-lg hover:shadow-clinic-600/20 ${stat.id ? 'cursor-pointer' : ''}`}
                >
